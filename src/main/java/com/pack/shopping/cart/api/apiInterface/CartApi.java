...
public interface CartApi {
    ...
    @GetMapping(value = "/api/carts/{customerId}", produces = { MEDIA_TYPE })
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 404, message = "Given customer ID doesn't exist") })
    @GetMapping(value = "/api/carts/{customerId}/items", produces = { MEDIA_TYPE })
    ...
    @GetMapping(value = "/api/carts/{customerId}/items/{itemId}", produces = { MEDIA_TYPE })
    default ResponseEntity<ItemDTO> getCartItemsByItemId(
            @ApiParam(value = "Customer Identifier", required = true) @PathVariable("customerId") String customerId,
            @ApiParam(value = "Item (product) Identifier", required = true) @PathVariable("itemId") String itemId) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType : MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf(MEDIA_TYPE))) {
                    String exampleString = "{ \"itemId\" : \"itemId\", \"unitPrice\" : 0.0, \", \"quantity\" : 0, \"imageUrl\" : \"imageUrl\", \"name\" : \"name\", \"description\" : \"description\" }";
    ...
    @ApiOperation(value = "Adds an item in shopping cart", nickname = "addCartItemsByCustomerId", notes = "Adds an item to the shopping cart if it doesn't already exist, or increment quantity by the specified number of items if it does.", response = ItemDTO.class, responseContainer = "List")
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Item added successfully", response = ItemDTO.class, responseContainer = "List"),
            @ApiResponse(code = 404, message = "Given customer ID doesn't exist") })
    @PostMapping(value = "/api/carts/{customerId}/items", produces = { MEDIA_TYPE }, consumes = { MEDIA_TYPE })
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Item added successfully", response = ItemDTO.class, responseContainer = "List"),
            @ApiResponse(code = 404, message = "Given customer ID doesn't exist") })
    @PutMapping(value = "/api/carts/{customerId}/items", produces = { MEDIA_TYPE }, consumes = { MEDIA_TYPE })
    ...
    @DeleteMapping(value = "/api/carts/{customerId}/items/{itemId}")
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 404, message = "Given customer ID doesn't exist") })
    @DeleteMapping(value = "/api/carts/{customerId}")
